...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    {
        n = 0;
        n = fread (buffer + *length, 1, BYTES, file);
        if (n != BYTES)
        {
        ...
        *length += n;
        buffer = realloc (buffer, *length + BYTES);
        if (buffer == NULL)
            return false;
    }
    *length += n;
...
const char* lookup(const char* path)
{
    ...
    if (strncasecmp (file, ".js", 3) == 0)
        return "text/javascript";
    else if (strncasecmp (file, ".css", 4) == 0)
        return "text/css";
    else if (strncasecmp (file, ".gif", 4) == 0)
        return "image/gif";
    else if (strncasecmp (file, ".ico", 4) == 0)
        return "image/x-icon";
    else if (strncasecmp (file, ".jpg", 4) == 0)
        return "image/jpeg";
    else if (strncasecmp (file, ".php", 4) == 0)
        return "text/x-php";
    else if (strncasecmp (file, ".png", 4) == 0)
        return "image/png";
    else if (strncasecmp (file, ".html", 5) == 0)
        return "text/html";
    return NULL;
}
...
{
    char* buf1 = strchr (line, ' ');
    if (buf1 == NULL)
    {
        error (400);
        return false;
    }
    char* buf2 = strchr (buf1 + 1, ' ');
    if (buf2 == NULL)
    {
        error (400);
        return false;
    }
    char* buf3 = strchr (buf2 + 1, ' ');
    if (buf3 != NULL)
    {
        error (400);
        return false;
    ...
    char* method = malloc (buf1 - line + 1);
    strncpy (method, line, buf1 - line);
    method [buf1 - line] = '\0';
    char* requestTarget = malloc (buf2 - buf1);
    strncpy (requestTarget, buf1 + 1, buf2 - buf1);
    requestTarget [buf2 - buf1 - 1] = '\0';
    char* HTTPVersion = malloc (buf3 - buf2);
    strncpy (HTTPVersion, buf2 + 1, buf3 - buf2 - 1);
    HTTPVersion [buf3 - buf2 - 1] = '\0';
    ...
    else if (strncmp (HTTPVersion, "HTTP/1.1", 8) != 0 || strlen(HTTPVersion) != 8)
    {
        free (method);
        free (requestTarget);
        free (HTTPVersion);
    ...
    else
    {
        ...
        else
        {
            ...
            if (q - end == 1)
                ...
                strncpy (query, q + 1, end - q);
            strncpy (abs_path, requestTarget, q - requestTarget);
            abs_path [q - requestTarget] = '\0';
        }
        free (method);
        free (requestTarget);
        free (HTTPVersion);
